...
def segment_file(file_path, smooth_w=5, perc=75, dist_frac=0.3):
    df = pd.read_csv(
        file_path, sep=r"\s+", header=None, names=["ax", "ay", "az", "gx", "gy", "gz"]
    )
...
if __name__ == "__main__":
    ...
    for fname in sorted(
        os.listdir(data_dir), key=lambda f: int(os.path.splitext(f)[0])
    ):
        ...
        for i, ch in enumerate(channels):
            ...
            for j, s in enumerate(starts):
                ...
                ax.axvline(s, color="tab:orange", linestyle="--", alpha=0.7, label=lbl)
            ax.set_title(ch)
            ax.grid(True)
